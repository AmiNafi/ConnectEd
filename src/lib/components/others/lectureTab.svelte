<script lang="ts">
	...
	let selected: lecture;

	$: {
		filteredData = tabData.filter((lecture) => {
			return (
				lecture.lectureName.toLowerCase().includes(name.toLowerCase()) ||
				(lecture.description && lecture.description.toLowerCase().includes(name.toLowerCase()))
			);
		});
	...
	$: {
		...
		if (form?.success == 'lectureUpload') {
			...
			tabData = data.courseData[0].lectures as lecture[];
			...
			clearTimeout(timerId);
			data.courseData[0].lectures.splice(selectedIndex, 1);
			tabData = data.courseData[0].lectures as lecture[];
			...
			clearTimeout(timerId);
			data.courseData[0].lectures = tabData;
			tabData = tabData;
		...
		{#each filteredData as item, i (i)}
			<Table.Row>
				<Table.Cell class="font-medium"
					><a
						href={item.lectureLink + '?' + timestamp}
		...
		>
			...
			<Dialog.Content class="sm:max-w-[425px]">
				<Dialog.Header>
					...
					<Dialog.Description>
						Edit lecture. Click save when you're done.
						{#if showMessage}
							<div
								class="mb-10 w-[300px] rounded-sm border-2 border-green-500 bg-green-100 px-2 py-2 text-base text-green-500"
							>
					...
					<div class="grid gap-4 py-4">
						<div class="grid grid-cols-4 items-center gap-4">
							...
							<Input
								...
								name="lectureName"
								class="col-span-3"
								bind:value={selected.lectureName}
								disabled={isLoading}
							/>
						</div>
