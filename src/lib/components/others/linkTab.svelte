<script lang="ts">
	...
	$: {
		filteredData = tabData.filter((item) => {
			return (
				item.linkName.toLowerCase().includes(name.toLowerCase()) ||
	...
	$: {
		...
		if (form?.success == 'linkUpload') {
			...
			tabData = data.courseData[0].links as link[];
			...
			clearTimeout(timerId);
			data.courseData[0].links.splice(selectedIndex, 1);
			tabData = data.courseData[0].links as link[];
			...
			clearTimeout(timerId);
			data.courseData[0].link = tabData;
			tabData = tabData;
						...
						>
							...
							<div class="grid gap-4 py-4">
								<div class="grid grid-cols-4 items-center gap-4">
									<Label class="text-right">Link Name</Label>
									<Input
										required
										id="linkName"
										name="linkName"
...
<Table.Root class="m-5 max-w-[1000px]">
	<Table.Caption>A list of your links.</Table.Caption>
	<Table.Header>
		<Table.Row>
			<Table.Head>Link name</Table.Head>
		...
		>
			...
			<Dialog.Content class="sm:max-w-[425px]">
				<Dialog.Header>
					...
					<Dialog.Description>
						Edit link. Click save when you're done.
						{#if showMessage}
							<div
								class="mb-10 w-[300px] rounded-sm border-2 border-green-500 bg-green-100 px-2 py-2 text-base text-green-500"
							>
